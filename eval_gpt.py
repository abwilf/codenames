...
def eval_gpt(model, tokenizer, lm, dataset_file=None, out_dir=None, device=-1, cache_dir=None, reader=None):
    task = reader
    assert 'gpt' in lm, 'Must be gpt model, not roberta. See src/evaluate_RoBERTa.py for that'
    if lm != 'gpt2-large':
        model_path = ['gpt2']+lm.split('/')[-1:]+[task]
        model_path = '_'.join([m for m in model_path if m != ''])
        out_dir = os.path.join(out_dir, model_path)
    else:
        out_dir = os.path.join(out_dir, 'gpt2_'+task)
    ...
    # Load the dataset
    instance_reader = INSTANCE_READERS[reader]()
    ...
    with open(dataset_file) as f_in:
        for line in tqdm(f_in, total=len(read_txt(dataset_file).split('\n'))):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_path", default=None, type=str, required=False, help="model_path (locally trained)")
    args = parser.parse_args()

    lm, dataset_file, out_dir, device, cache_dir, reader = args.lm, args.dataset_file, args.out_dir, args.device, args.cache_dir, args.reader

    device = torch.device(f'cuda:{device}') if device >= 0 else torch.device("cpu")
    if args.model_path is not None:
        model = AutoModelWithLMHead.from_pretrained('./results/test_train', local_files_only=True)
        tokenizer = AutoTokenizer.from_pretrained('./results/test_train', local_files_only=True)
    else:
        model, tokenizer = init_model(lm, device, cache_dir)
    model.to(device)
    model.eval()
    eval_gpt(model, tokenizer, lm, dataset_file, out_dir, device, cache_dir, reader)
